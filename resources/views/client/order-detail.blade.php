...
@section('content')
    ...
    <div class="container mt-5" style="width: 60%">
        <h3 class="text-center" style="font-size: 33px; font-weight: 600">CHI TIẾT ĐƠN HÀNG</h3>
        <div class="order-detail-container ">
                                ...
                                <div class="product-item d-flex" style="width: 100%;">
                                    ...
                                    <div class="product-details mx-3" style="flex-grow: 1;">
                                        <strong style="display: inline-block; max-width: 300px; word-wrap: break-word;">
                                            <a
                                                href="{{ route('productDetail', $item->productVariant->product->slug) }}">{{ $item->product_name }}</a>
                                        </strong> <br>
                                        @php
                                            $size = '';
                                            $color = '';
                                ...
                                @if ($order->status == '4')
                                    @if ($commentDataArray[$item->productVariant->product->id]['status'] == 'not_comment')
                                        <div class="review-button-container" style="text-align: right;">
                                            <a href="#"
                                                class="btn btn-primary btn-sm rounded-2 w-30 fw-normal text-capitalize"
                                                data-bs-toggle="modal" data-bs-target="#reviewModal"
                                                data-product-id="{{ $item->productVariant->product->id }}"
                                                data-order-id="{{ $order->id }}" data-user-id="{{ Auth::id() }}"
                                    ...
                                    @elseif($commentDataArray[$item->productVariant->product->id]['status'] == 'updated')
                                        <div class="review-button-container" style="text-align: right;">
                                            <a href="#"
                                                class="btn btn-primary btn-sm rounded-2 w-30 fw-normal text-capitalize"
                                                data-bs-toggle="modal" data-bs-target="#getModal"
                                                data-product-id="{{ $item->productVariant->product->id }}"
                                                data-order-id="{{ $order->id }}" data-user-id="{{ Auth::id() }}"
                                                {{-- data-comment="39" --}}
                                                data-comment="{{ $commentDataArray[$item->productVariant->product->id]['comment']['id'] ?? '' }}"
            ...
            <div class="order-total">
                ...
                <a href="{{ route('shop') }}" class="btn btn-sm mb-2 fw-normal text-capitalize"
                    style="height: 30px">Tiếp tục mua sắm</a>
            </div>
        </div>
    </div>
@endsection
                ...
                <div class="modal-body">
                    <div id="editCommentForm">
                        <form id="editForm" method="POST" action="">
                            ...
                            <input type="text" name="title" class="form-control mb-2" id="edit-title"
                                placeholder="Tiêu đề">
                    ...
                    <div class="review-inner">
                        <div class="spr-review d-flex w-100">
                            ...
                            <div class="spr-review-content flex-grow-1">
                                ...
                                <p class="updated-at text-muted" style="font-size: 0.8em;"></p>
                                <!-- Thêm dòng này để hiển thị thời gian cập nhật -->
                            </div>
                        </div>
                    </div>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editModal.addEventListener('show.bs.modal', function(event) {
                ...
                ratingStars.forEach(star => {
                    var starIcon = star.nextElementSibling.querySelector(
                        'i'); // Lấy icon sao kế bên input
                ...
                fetch(`/comments/show/${commentId}`)
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            ...
                            document.querySelector('#getModal .spr-review-header-title').textContent =
                                data.user_name;
                            document.querySelector('#getModal .spr-review-body').textContent = data
                                .comment;
                            ...
                            if (reviewLinkElement) {
                                ...
                                console.log(reviewLinkElement
                                    .innerHTML); // Kiểm tra nội dung đã được cập nhật
                            }

                            // Cập nhật avatar và thời gian cập nhật
                            document.getElementById('user-image').src = data.avatar; // Cập nhật avatar
                            document.querySelector('#getModal .updated-at').textContent =
                                `${data.updated_at}`; // Cập nhật thời gian
                        }
                    })
                    .catch(error => {
                        ...
                        document.querySelector('#getModal .spr-review-header-title').textContent =
                            'Không có đánh giá';
                        document.querySelector('#getModal .spr-review-body').textContent =
                            'Hiện tại chưa có đánh giá nào cho sản phẩm này.';
                        const reviewLinkElement = document.querySelector(
                            '#getModal .product-review span.reviewLink');
                        if (reviewLinkElement) {
