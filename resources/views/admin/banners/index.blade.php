                ...
                <div class="block-options">
                    @can('Thêm mới banner')
                        <div class="block-options-item">
                            <a href="{{ route('admin.banners.create') }}" class="btn btn-sm btn-alt-secondary"
                ...
                <table class="table table-hover align-middle table-striped  js-dataTable-full">
                    <thead>
                        <tr>
                            ...
                            @if (Auth::user()->can('Kích hoạt banner') || Auth::user()->can('Xóa banner') || Auth::user()->can('Chỉnh sửa banner'))
                                <th>Thao tác</th>
                            @endif
                        </tr>
                    </thead>
                            ...
                            <tr>
                                ...
                                <td>
                                    @foreach ($banner->images as $image)
                                        <img src="{{ asset('storage/' . $image->image) }}" alt="Banner Image"
                                            width="100">
                                    @endforeach
                                </td>
                                <td>
                                ...
                                @if (Auth::user()->can('Kích hoạt banner') || Auth::user()->can('Xóa banner') || Auth::user()->can('Chỉnh sửa banner'))
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <!-- ACTIVATE -->
                                            @can('Kích hoạt banner')
                                                @if (!$banner->is_active)
                                                    <form action="{{ route('admin.banners.activate', $banner->id) }}"
