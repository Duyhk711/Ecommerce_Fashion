    ...
    <div class="content">
        ...
        <div class="row items-push">
            ...
            <div class="col-6 col-lg-3">
                ...
                @elseif($orderDetail->payment_status == 'huy_thanh_toan')
                    <a class="block block-rounded block-link-shadow text-center h-100 mb-0" href="javascript:void(0)">
                        <div class="block-content py-5">
            ...
            <div class="col-6 col-lg-3">
                ...
                @elseif($orderDetail->status == '2' || $orderDetail->status == '3')
                    <a class="block block-rounded block-link-shadow text-center h-100 mb-0" href="javascript:void(0)">
                        <div class="block-content py-5">
                            <div class="item rounded-circle bg-xsmooth-lighter mx-auto mb-3">
                                <i class="fa fa-sync fa-spin text-xsmooth-dark"></i>
            ...
            <div class="col-6 col-lg-3">
                ...
                @elseif($orderDetail->status == '2' || $orderDetail->status == '1')
                    <a class="block block-rounded block-link-shadow text-center h-100 mb-0" href="javascript:void(0)">
                        <div class="block-content py-5">
                            <div class="item rounded-circle bg-xsmooth-lighter mx-auto mb-3">
                                <i class="fa fa-sync fa-spin text-xsmooth-dark"></i>
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default d-flex justify-content-between">
                ...
                @can('Chỉnh sửa trạng thái đơn hàng')
                    @if ($orderDetail->status == '4' || $orderDetail->status == 'huy_don_hang' || $orderDetail->status == '5')
                    ...
                    @else
                        <button class="btn btn-sm btn-primary" style="height: 30px;" data-bs-toggle="modal"
                            data-bs-target="#updateStatusModal" data-id="{{ $orderDetail->id }}"
                            data-status="{{ $orderDetail->status }}" title="Chỉnh sửa">
                            Chỉnh sửa trạng thái
                        </button>
                    @endif
                @endcan
                            ...
                            @foreach ($items as $item)
                                <tr>
                                    <td>
                                        <div class="d-flex">
                                            <div>
                                                <img src="{{ asset('storage/' . $item->variant_image) }}" width="80px"
                                                    alt="">
                                            </div>
                                            <div class="mx-2">
                                                <a
                                                    class="text-black fs-5"><strong>{{ $item->product_name }}</strong></a><br>
                                    ...
                                    <td class="text-end">
                                        {{ number_format(($item->variant_price_sale == 0 ? $item->variant_price_regular : $item->variant_price_sale) * 1000, 0, ',', '.') }}
                                        ₫</td>
                                    <td class="text-end">
                                        {{ number_format($item->quantity * ($item->variant_price_sale == 0 ? $item->variant_price_regular : $item->variant_price_sale) * 1000, 0, ',', '.') }}
                                        ₫</td>
                                </tr>
                            @endforeach
                            <tr>
                                ...
                                <td class="text-end">
                                    {{ number_format(($orderDetail->total_price + $orderDetail->discount) * 1000, 0, ',', '.') }}₫
                                </td>
                            </tr>
                            <tr>
                                <td colspan="3" class="text-end"><strong>Giảm giá:</strong></td>
                                <td class="text-end">{{ number_format($orderDetail->discount * 1000, 0, ',', '.') }}₫
                                </td>
                            </tr>
                            ...
                            @if ($statusChanges->isNotEmpty())
                                @foreach ($statusChanges as $change)
                                    <tr>
                                        ...
                                        <td
                                            class="{{ $change->new_status == '4' ? 'text-success' : ($change->new_status == 'huy_don_hang' ? 'text-danger' : 'text-warning') }}">
                                            <strong>
                                                @if ($change->new_status == '1')
                                                    Đơn hàng đã đặt thành công
                                                @elseif($change->new_status == '2')
                                                    Đơn hàng đã được xác nhận, đang chuẩn bị đơn hàng giao cho đơn vị vận
                                                    chuyển
